...
sa  <- rio::import("Bases/Base_FIES_2023.csv")[,c(1:9,18:20)]
sa_h <- sa %>% distinct(ID, .keep_all = TRUE)

ech23 <- merge(sa_h, ech2023, by="ID", all=T)
...
##Comedor
ech2023 <- ech2023 %>% dplyr::mutate(comedor = case_when(e559==1|e582==1 ~ 1, e559!=1&e582!=1 ~ 0)) 

##Canasta
ech2023 <- ech2023 %>% dplyr::mutate(canasta = case_when(e59==1 ~ 1, e59!=1 ~ 0)) 

##Política alimentaria (excluídas las canastas)
ech2023 <- ech2023 %>% dplyr::mutate(pol_alim_scan = case_when(e559==1|e582==1|e560==1 ~ 1, 
                                                               e559!=1&e582!=1&e560!=1 ~ 0)) 

##Política alimentaria (incluídas las canastas)
ech2023 <- ech2023 %>% dplyr::mutate(pol_alim = case_when(e559==1|e582==1|e560==1|e59==1 ~ 1, 
                                                          e59!=1&e559!=1&e582!=1&e560!=1 ~ 0)) 

# Indigencia
ech2023 <- ech2023 %>% dplyr::mutate(indig = indig06)

# Sexo
ech2023 <- ech2023 %>% dplyr::mutate(bc_pe2 = e26)
ech2023 <- ech2023 %>% dplyr::mutate(sexo1 = case_when(e26==1 ~ 1, e26==2 ~ 0))
ech2023 <- ech2023 %>% dplyr::mutate(sexo2 = case_when(e26==2 ~ 1, e26==1 ~ 0))
                                                                          
# Ascendencia afro
ech2023 <- ech2023 %>% dplyr::mutate(bd_e29_1 = e29_1)
ech2023 <- ech2023 %>% dplyr::mutate(afro1 = case_when(bd_e29_1==1 ~ 1, bd_e29_1==2 ~ 0))
ech2023 <- ech2023 %>% dplyr::mutate(afro2 = case_when(bd_e29_1==2 ~ 1, bd_e29_1==1 ~ 0))
                                                                          
# Edad
ech2023 <- ech2023 %>% dplyr::mutate(tramo1 = case_when(e27>=0 & e27 <=5 ~ 1,
                                                  e27>=6 & e27 <=12 ~ 2,
                                                  e27>=13 & e27 <=17 ~ 3,
                                                  e27>=18 & e27 <=29 ~ 4,
                                                  e27>=30 & e27 <=64 ~ 5,
                                                  e27>=65 ~ 6))
ech2023 <- ech2023 %>% dplyr::mutate(tramo2 = case_when(e27>=0 & e27 <=5 ~ 1,
                                                  e27>=6 & e27 <=12 ~ 2,
                                                  e27>=13 & e27 <=17 ~ 3,
                                                  e27>=18 & e27 <=29 ~ 4,
                                                  e27>=30 ~ 5))
...
ech2023 <- ech2023 %>% dplyr::mutate(tramo1_1 = case_when(e27>=0 & e27 <=5 ~ 1, e27>5 ~ 0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo1_2 = case_when(e27>=6 & e27 <=12 ~ 1, e27<6|e27>12 ~ 0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo1_3 = case_when(e27>=13 & e27 <=17 ~ 1, e27<13|e27>17~0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo1_4 = case_when(e27>=18 & e27 <=29 ~ 1, e27<18|e27>29~0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo1_5 = case_when(e27>=30 & e27 <=64 ~ 1, e27<30|e27>64~0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo1_6 = case_when(e27>=65 ~ 1, e27<65 ~ 0))

ech2023 <- ech2023 %>% dplyr::mutate(tramo2_1 = case_when(e27>=0 & e27 <=5 ~ 1, e27>5 ~ 0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo2_2 = case_when(e27>=6 & e27 <=12 ~ 1, e27<6|e27>12 ~ 0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo2_3 = case_when(e27>=13 & e27 <=17 ~ 1, e27<13|e27>17~0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo2_4 = case_when(e27>=18 & e27 <=29 ~ 1, e27<18|e27>29~0))
ech2023 <- ech2023 %>% dplyr::mutate(tramo2_5 = case_when(e27>=30  ~ 1, e27<30~0))
...
ech2023_svy   <- srvyr::as_survey_design(ech2023, ids = ID, weights = W_ANO)
...
c_e_sexo <- sapply(ech2023_svy$variables %>% filter(comedor==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(comedor==1) %>%
...
c_e_edad <- sapply(ech2023_svy$variables %>% filter(comedor==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(comedor==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
c_e_afro <- sapply(ech2023_svy$variables %>%  filter(comedor==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(comedor==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
proyecciones23 = subset(proyecciones, Año==2023)[,2]
...
AÑO	         <- 2023
fecha_cat    <- ""
VALOR	       <- c(c_ano*proyecciones23, c_sexo[1]*c_ano*proyecciones23, c_sexo[2]*c_ano*proyecciones23, 
                  c_edad[1]*c_ano*proyecciones23,c_edad[2]*c_ano*proyecciones23,c_edad[3]*c_ano*proyecciones23,
                  c_edad[4]*c_ano*proyecciones23,c_edad[5]*c_ano*proyecciones23,c_edad[6]*c_ano*proyecciones23, 
                  c_afro[1]*c_ano*proyecciones23,c_afro[2]*c_ano*proyecciones23)
FUENTE	     <- "Encuesta Continua de Hogares 2023, INE"
RESPONSABLE	 <- "S. Katzkowicz"
JERARQUIA	   <- 1
JERARQUIA_CAT<- ""
...
var_int <- c("sexo1", "sexo2")
c_e_sexo <- sapply(ech2023_svy$variables %>% filter(pol_alim_scan==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(pol_alim_scan==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
var_int <- c("tramo1_1", "tramo1_2", "tramo1_3", "tramo1_4", "tramo1_5", "tramo1_6")
c_e_edad <- sapply(ech2023_svy$variables %>% filter(pol_alim_scan==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(pol_alim_scan==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
var_int <- c("afro1", "afro2")
c_e_afro <- sapply(ech2023_svy$variables %>%  filter(pol_alim_scan==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(pol_alim_scan==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
var_int <- c("sexo1", "sexo2")
c_e_sexo <- sapply(ech2023_svy$variables %>% filter(canasta==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(canasta==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
AÑO	         <- 2023
fecha_cat    <- ""
VALOR	       <- c(c_ano*proyecciones23, c_sexo[1,1]*c_ano*proyecciones23, c_sexo[2,1]*c_ano*proyecciones23)
FUENTE	     <- "Encuesta Continua de Hogares 2023, INE"
RESPONSABLE	 <- "S. Katzkowicz"
JERARQUIA	   <- 1
JERARQUIA_CAT<- ""
...
var_int <- c("sexo1", "sexo2")
c_e_sexo <- sapply(ech2023_svy$variables %>% filter(pol_alim==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(pol_alim==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
var_int <- c("tramo1_1", "tramo1_2", "tramo1_3", "tramo1_4", "tramo1_5", "tramo1_6")
c_e_edad <- sapply(ech2023_svy$variables %>% filter(pol_alim==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(pol_alim==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
var_int <- c("afro1", "afro2")
c_e_afro <- sapply(ech2023_svy$variables %>%  filter(pol_alim==1) %>% select(var_int), function(x){
  ech2023_svy %>% filter(pol_alim==1) %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
alim_2023 <- rbind(alim_01, alim_02, alim_03, alim_04, alim_05, alim_06, alim_07, alim_08, alim_09)

rio::export(alim_2023, "alim_2023.xlsx" )
