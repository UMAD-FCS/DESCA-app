...
# Salario mínimo nacional enero 2024
...
base <- base %>% dplyr::mutate(p_tus = ifelse(e584 == 1, 1, 0))
...
base <- base %>% dplyr::mutate(subsidio_def = g148_3 /  ipc_ene2024)
...
base <- base %>% dplyr::mutate(y_jub_def = y_jub /  ipc_ene2024)
...
base <- base %>% dplyr::mutate(y_pens_def = y_pens /  ipc_ene2024)
...
base <- base %>% dplyr::mutate(y_transf = e584_1 + g257)
base <-  base %>%
  group_by(ID) %>%
...
ss_2024 <- rbind(ss_01, ss_02, ss_03, ss_04, ss_05, ss_06, ss_07, ss_08, ss_09, ss_10, ss_11, ss_12, ss_13, ss_14, ss_15, ss_16, ss_17, ss_18, ss_19, ss_20)

rio::export(ss_2024, "Tabulados/Tabulados 2024/ss_2024.xlsx" )
